...
<tr>
	<th width="140"><?php echo L('二维码');?>:</th>
	<td><div class="barcode_box"  ><!-- <div id="pic_aid_box" style="display:none;"></div> -->
	<input type="text" id="barcode" name="barcode"  value="<?php echo ($user_config[barcode][value]); ?>" class="input-text" size="40"/>
	<input type="button" class="button" value="上传" onclick="javascript:swfupload('barcode_uploadfile','barcode','<?php echo L(uploadfiles);?>',1,1,0,10,'jpeg,jpg,png,gif',5,230,'<?php echo ($yourphp_auth); ?>',yesdo,nodo)" />  <a href="javascript:void(0);" onclick="showpicbox($('#barcode_box').val());"> 预览</a></div>
	</td>	
</tr>
...
<?php if(is_array($user_config)): $i = 0; $__LIST__ = $user_config;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($vo[varname]!='barcode' ) : ?>
<tr>
	<th width="140"><?php echo ($vo["info"]); ?>:</th>
	<td><input type="text" class="input-text"  name="<?php echo ($vo["varname"]); ?>" value="<?php echo ($vo["value"]); ?>" size="40"> &nbsp;<?php echo ($vo["varname"]); ?> </td>
</tr>
