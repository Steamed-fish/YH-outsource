...
<div class="container case-detail">
  <div class='row'>
    <div class="col-sm-9 col-md-9 col-xs-12 c-left">
      ...
      <div class="xiaye pd f14 lh22 margin-T-30 margin-B-30">
        <?php if($prePage) : ?><div><?php echo L('上一篇');?>:<a class="" href="<?php echo ($prePage["url"]); ?>"><?php echo ($prePage["title"]); ?></a></div><?php else : endif;?>
        <?php if($nextPage) : ?><div><?php echo L('下一篇');?>:<a class="" href="<?php echo ($nextPage["url"]); ?>"><?php echo ($nextPage["title"]); ?></a></div><?php else : endif;?>
      </div>
    </div>
    <div class="col-sm-3 col-md-3 col-xs-12 c-right">
      <div class="col-r-title"><?php echo L('其它案例');?></div>
      <ul>
        <?php  $_result=M("Project")->field("*")->where(" 1  and lang=3 AND status=1  AND  thumb !='' ")->order("id desc")->limit("5")->select();; if ($_result): $i=0;foreach($_result as $key=>$r):++$i;$mod = ($i % 2 );?><li class="row">
                <a href='<?php echo ($r["url"]); ?>' >
                  <div class="col-sm-5 col-md-5 col-xs-4 nopad">
                    <img alt='<?php echo ($r["title"]); ?>' src='<?php echo (thumb2($r["thumb"],219,135,1)); ?>' />
                  </div>
                  <div class="col-sm-7 col-md-7 col-xs-8 padd-T-15">
