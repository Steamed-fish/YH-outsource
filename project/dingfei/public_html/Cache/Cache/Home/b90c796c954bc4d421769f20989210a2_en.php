							...
							<div class="pan-pro">
									<div class="padding-null has-mask">
										...
										<div id="owl-example<?php echo ($c["id"]); ?>" class="owl-carousel">
											...
											<?php  $_result=M("Product")->field("*")->where(" 1  and lang=2 AND status=1  AND catid in(" . $c[id] . ")")->order("id desc")->limit("4")->select();; if ($_result): $i=0;foreach($_result as $key=>$r):++$i;$mod = ($i % 2 );?><div>
	...
	<div class="container">
		...
		<div class="row">
			<div class="owl-carousel recommand-carousel" id="case-list">
				<?php  $_result=M("Solution")->field("*")->where(" 1  and lang=2 AND status=1   AND posid =11 AND  thumb !='' ")->order("id desc")->limit("16")->select();; if ($_result): $i=0;foreach($_result as $key=>$r):++$i;$mod = ($i % 2 );?><div class="overout-outer li">
...
<div class="gray-light">
	<div class="container">
		...
		<div class="row">
			...
			<?php  $_result=M("Article")->field("*")->where(" 1  and lang=2 AND status=1  AND  thumb !='' ")->order("id desc")->limit("3")->select();; if ($_result): $i=0;foreach($_result as $key=>$r):++$i;$mod = ($i % 2 );?><div class="col-sm-4 col-md-4 col-xs-12 marginB20">
				<a href='<?php echo ($r["url"]); ?>' >
...
<div class="container">
	...
	<div class="row">
			<?php  $_result=M("Link")->field("*")->where(" status = 1  and lang=2 and typeid=3 and  linktype=2")->order("id desc")->limit("8")->select();; if ($_result): $i=0;foreach($_result as $key=>$r):++$i;$mod = ($i % 2 );?><div class="pan-brand">
	...
	@media all and (-ms-high-contrast: none), (-ms-high-contrast: active) {
		...
		<div class="container">
			...
			<div class="row friendlink">
				<b><?php echo L('友情链接');?>: </b>
				<?php  $_result=M("Link")->field("*")->where(" status = 1  and lang=2 and typeid=2 and  linktype=1")->order("id desc")->limit("10")->select();; if ($_result): $i=0;foreach($_result as $key=>$r):++$i;$mod = ($i % 2 );?><a href="<?php echo ($r['siteurl']); ?>" target="_blank"><?php echo ($r['name']); ?></a><?php endforeach; endif;?>
			</div>
		</div>
		<div class="copyright">
			<div>
