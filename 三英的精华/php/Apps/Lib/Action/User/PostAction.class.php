...
class PostAction extends BaseAction
{
    ...
    public function insert()
    {
		...
		//  	$body .= '<div>联系电话  :    '.$_POST["telephone"].'</div>';
		     ...
		     $modName = $this->module[$this->moduleid]['name'];
		     //if(strtolower($modName) == 'Freight'){
		     	$body .= '<div>内容  :    '.$_POST["content"].'</div>';
		     	$body .= '<div>运输方式  :    '.$_POST["way"].'</div>';
		     	$body .= '<div>地区  :    '.$_POST["guojia"].'</div>';
		     	$body .= '<div>时间  :    '.$_POST["time"].'</div>';
		     	$body .= '<div>货物重量  :    '.$_POST["weight"].'</div>';
		     	$body .= '<div>名字  :    '.$_POST["name"].'</div>';
		     	$body .= '<div>电话  :    '.$_POST["dianhua"].'</div>';
		     	$body .= '<div>邮箱  :    '.$_POST["email"].'</div>';
		     //}
