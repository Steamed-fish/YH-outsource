...
class Page extends Think {
	...
	public function  show(){
		...
		//add by zeb 获取上一页和下一页url
		$this->prePage = $this->pageurl($urlrule, $pre_page,$this->parameter);
		$this->nextPage = $this->pageurl($urlrule,$next_page,$this->parameter);
		//end


		$output = '';
		/*$output .= '<a class="a1">'.$this->totalRows.L(' ').'</a>';*/
		$output .= '<a class="a2" href="'.$this->pageurl($urlrule, 1,$this->parameter).'">'.L('&lt&lt First').'</a>';
		$output .= '<a class="a3" href="'.$this->pageurl($urlrule, $pre_page,$this->parameter).'">'.L('<').'</a>';
		...
		$output .='<a class="a4" href="'.$this->pageurl($urlrule,$next_page,$this->parameter).'">'.L('>')."</a>"; 
		$output .='<a class="a5" href="'.$this->pageurl($urlrule,$this->totalPages,$this->parameter).'">'.L('Last &gt;&gt;')."</a>";
		return $output;
	}
