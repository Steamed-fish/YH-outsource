...
class Model {
    ...
    protected function _after_insert($data,$options) {
        ...
        $this->fieldsConfig = F($moduleid.'_Field');
        ...
        if ($this->fieldsConfig)
        {
            ...
            {
                if ('images' == $config['type'] || 'files' == $config['type'])
                {
                    $arr = explode(':::', $data[$field]);
                    foreach ($arr as $a)
                    {
                        list($path, $name) = explode('|', $a);
                        $attachment[] = $path;
                    }
                }elseif ('image' == $config['type'] || 'file' == $config['type']) {
            ...
            if ($attachment)
            {
                M('Attachment')->where(array('filepath' => array('in', $attachment), 'status' => 0))->save(array('status' => 1));
            }
        }
    }
