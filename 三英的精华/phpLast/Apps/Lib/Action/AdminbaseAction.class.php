...
class AdminbaseAction extends Action
{
	...
	function _list($modelname, $map = '', $sortBy = '', $asc = false ,$listRows = 15) {
		...
		*/
        ...
        if($catid) {
            if($relationId = hasCatalogField($this->moduleid))
            {
		...
		if ($count > 0) {
			...
			$myfield = (empty($field) || $field == '*') ? array() : explode(',', $field);
			$myfield_array = array();
			...
			if (count($myfield) > 0) {
			    $default_filed = array(
			        ...
			        'username' => array('name' => '用户名称'),
			        'updatetime' => array('name' => '更新时间'),
			    );
			    foreach ($myfield as $key) {
			        if (isset($this->fields[$key])) {
			            $myfield_array[] = array(
			                'name' => $this->fields[$key]['name']
			            );
			        } elseif (isset($default_filed[$key])) {
			            $myfield_array[] = array(
			                'name' => $default_filed[$key]['name']
			            );
			        }
			    }
			    $voList = $this->cureInputLabel($voList);
			}
			
			$this->assign('myfield', $myfield_array);
			$this->assign ( 'list', $voList );
			$this->assign ( 'page', $page );
			return array('list'=>$voList, 'page'=>$page, 'count'=>$count);
		}
		return;
	}
	 ...
	 * @param $voList
	 */
	protected function cureInputLabel($voList)
	{
	    if ($voList) {
	                ...
	                if (isset($this->fields[$k])) {
	                    ...
	                    switch ($info['type']) {
	                        ...
	                        case 'radio':
	                            ...
	                            $vo[$info['field']] = isset($optionsarr[$v]) ? $optionsarr[$v] : $v; //
	                    }
	                }
	            }
	        }
...
/**
    ...
    public function excel(){//导出Excel
        ...
        $ids=$_POST['ids'];
        if($ids){
        	$where=array('id'=>array('in',$ids),'lang'=>APP_LANG);
        }else{
        	$where=array('lang'=>APP_LANG);
        }
