...
class OrderAction extends BaseAction
{
	...
	public function confirmOrder()
	{
		...
		if ($this->_userid > 0)
		{
			...
			if (empty($productid))
			{
				$product=$this->getProductByCart($cartid);
				$this->assign('type', 2);
			}else {
			...
			$shipping = $this->getShippingList();
			
			$this->assign('products', $product);
			$this->assign('number', $allNumber);
			$this->assign('amount', $amount);
			$this->assign('shipping', $shipping);
			$this->assign('realPay', $amount);
			$this->assign('isGuest', false);
	...
	{
		$shipping = M('shipping')->where(array('status'=>'1'))->order('listorder asc')->field('id,name,first_price as price')->select();
		if ($shipping)
		{
			return $shipping;
		}else {
			return array(array('id'=>0,'name'=>'普通快递', 'price'=>0));
		}
	}
	
	private function getProductByCart($cartid)
	{
		$cartModel = M('Cart');
		if (is_array($cartid))
		{
			...
			$cartModel->where(array('userid'=>intval(cookie('userid')), 'id'=>array('in', $cartid)));
		}else {
			$cartModel->where(array('userid'=>intval(cookie('userid')), 'id'=>$cartid));
		}
		$cartModel->field('id,product_id as productid,product_thumb as thumb,product_name as title,price,number,product_url as url');
		$result = $cartModel->select();
	...
	private function getProductById($productid)
	{
		...
		$productModel->field('0 as id,id as productid,title,price,thumb,1 as number,url');
