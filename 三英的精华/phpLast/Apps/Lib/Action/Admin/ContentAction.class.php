...
class ContentAction extends AdminbaseAction
{
	...
	{  
		...
		if (false !== $list) {
			/**
			 * @todo 当被更新数据的表为商品表时，如果商品价格被更改，需要同修改商品在购物车中的价格信息
			 * @var Ambiguous $id
			 */
			if (MODULE_NAME == 'Product' && $olddata['price'] != $_POST['price'])
			{
				$this->updateCart($_POST['id'], $_POST['price']);
			}
	 ...
	 * @param float $price
	 */
	private function updateCart($productid,$price)
	{
		$cart = M('Cart');
		$cart->where(array('product_id'=>$productid))->save(array('price'=>$price,'product_price'=>$price));
	}
