...
class Form extends Think {
	...
	public function title($info,$value){
		...
		$thumbstr ='<div class="thumb_box"  id="thumb_box"><div id="thumb_aid_box"></div>
				<a href="javascript:swfupload(\'thumb_uploadfile\',\'thumb\',\''.L('uploadfiles').'\','.$this->isadmin.',1,1,1,\'jpeg,jpg,png,gif\','.$info['setup']['upload_maxsize'].','.$info['moduleid'].',\''.$yourphp_auth.'\',yesdo,nodo)"><img src="'.$thumb_ico.'" id="thumb_pic" ></a><br> <input type="text"  id="thumb_alt" name="thumb_alt"  value="'.$this->data['thumb_alt'].'" placeholder="'.L('thumb_alt').'" /><br>
				<input type="button" value="'.L('clean_thumb').'" onclick="javascript:clean_thumb(\'thumb\');" class="button" />
	...
	public function text($info,$value){
        ...
        isset($info['setup']['hint']) && $parseStr.=$info['setup']['hint'];
		return $parseStr;
	}
	...
	public function images($info,$value){
		...
		if($value){
			...
			if(is_array($options)){
			    if (!$info['setup']['description'])
			    {
    				foreach($options as  $r) {
    						$v = explode("|",$r);
    						$k = trim($v[1]);
    						$optionsarr[$k] = $v[0];
    						$data .='<div id="' . $field . 'uplistd_'.$i.'"><input type="text" size="50" class="input-text" name="'.$field.'[]" value="'.$v[0].'"  /> <input type="text" class="input-text" name="'.$field.'_name[]" value="'.$v[1].'" size="30" /> &nbsp;<a href="javascript:remove_this(\'' . $field . 'uplistd_'.$i.'\');">'.L('remove').'</a> </div>';
    						$i++;
    				}
			    }else {
			        foreach($options as  $r) {
			            ...
			            $k = trim($d[0]);
			            $optionsarr[$k] = $v[0];
			            $data .='<div id="uplistd_'.$i.'"><input type="text" size="50" class="input-text" name="'.$field.'[]" value="'.$v[0].'"  /> '
			                .'<input type="text" class="input-text" name="'.$field.'_name[]" value="'.$k.'" size="30" /> '
			                .'&nbsp;<a href="javascript:remove_this(\'uplistd_'.$i.'\');">'.L('remove').'</a> '
			                .'&nbsp;<a href="javascript:void(0);" onclick="showpicbox($(\'#uplistd_'.$i.'\').children(\':first\').val());">预览</a>'
			                .'<div style="padding-top: 2px;padding-bottom: 5px;">'
			                .'    <textarea name="'.$field.'_desc[]" style="width: 529px;" placeholder="请设置图片描述信息">'.trim($d['1']).'</textarea>'
		                    .'</div>'
			                .'</div>';
			            $i++;
			        }
		...
		$up_images = empty($info['setup']['up_images']) ? 'up_images' : $info['setup']['up_images'];
		$desc_input = $info['setup']['description'] ? '<input type="hidden" name="'.$field.'_desc[]" value=""/>' : '';
		...
		<div id="'.$field.'_images" class="imagesList"><input type="hidden"  name="'.$field.'[]" value=""/><input type="hidden"   name="'.$field.'_name[]" value="" />'.$desc_input.$data.'</div>
		</fieldset>
		<div class="c"></div>
        ...
        if ($value)
        {
            ...
            if(is_array($options))
            {
                foreach($options as  $r) {
                    ...
                    $k = trim($v[1]);
                    $optionsarr[$k] = $v[0];
                    $data .='<div id="' . $field . '_uplistd_'.$i.'"><input type="text" size="50" class="input-text" name="'.$field.'[]" value="'.$v[0].'"  /> <input type="text" class="input-text" name="'.$field.'_name[]" value="'.$v[1].'" size="30" /> &nbsp;<a href="javascript:remove_this(\'' . $field . '_uplistd_'.$i.'\');">'.L('remove').'</a> </div>';
                    $i++;
                }
            }
        }
