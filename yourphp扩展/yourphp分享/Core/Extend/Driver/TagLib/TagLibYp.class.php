...
class TagLibYp extends TagLib
{
	protected $tags   =  array(
		...
		'share' => array('attr'=>'url,title','close'=>0),
    );
	...
	public function _share($attr,$content) {
		$tag    = $this->parseXmlAttr($attr,'share');
		//定义数据查询的结果存放变量
		$url = !empty($tag['url'])?$tag['url']:'http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']; 
		...
		$share = M('Share')->where($where)->select();
		
		if(empty($share)) return  '';
		
		$content = '';
		foreach ($share as $key => $value) {
			print_r($value[source]);
			$content.= str_replace(array('{$url}','{$title}'),array($url,$title), $value[source]);
		}

 		$Tpl =M('Config')->where('varname="tpl"')->find();
 		
 
		$Tpl = TMPL_PATH.'Home/'.THEME_NAME.'/Share_'.$Tpl['value'].'.html';
		//$flashpath = TMPL_PATH.C('DEFAULT_THEME_NAME').'/Public/flash/';
		$html = file_get_contents($Tpl);
		$html = str_replace(array('{$content}'),array($content),$html);
