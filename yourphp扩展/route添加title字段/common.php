...
function make_urlrule($url,$lang,$action,$MOREREQUEST=''){
		...
		$REQUEST=str_replace(array('{$parentdir}','{$module}','{$moduleid}','{$catdir}','{$year}','{$month}','{$day}','{$catid}','{$id}','{$title}','{$page}'),array('','module','moduleid','catdir','year','month','day','catid','id','title',C('VAR_PAGE')),$matches[0]);
		$rule=str_replace(array('{$parentdir}','{$module}','{$moduleid}','{$catdir}','{$year}','{$month}','{$day}','{$catid}','{$id}','{$title}','{$page}','/',C('URL_HTML_SUFFIX')),array('','([A-Z]{1}[a-z]+)','(\d+)','([\w^_]+)','(\d+)','(\d+)','(\d+)','(\d+)','(\d+)','([\w^_]+)','(\d+)','\/',''),$url);
...
function geturl($cat,$data='',$Urlrule=''){
		...
		if(empty($urls)){
			...
			$langurl = $lang ? $lang.'/' : '';
			if($id){
				$urls = str_replace(array('{$parentdir}','{$module}','{$moduleid}','{$catdir}','{$year}','{$month}','{$day}','{$catid}','{$id}','{$title}'),array($parentdir,$module,$moduleid,$catdir,$year,$month,$day,$catid,$id,$title),$showurlrule);
			}else{
